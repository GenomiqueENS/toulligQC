...
class TestSequencingSummaryExtractorWholeConfig(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        data_test = np.array(
            [
                [20, 8.92175, 1.220750, True, 257, 14.379014999999999, "barcode12"],
    ...
    def test_load_sequencing_summary_data_assert_equal(self):
        ...
        testing.assert_frame_equal(
            self.expected_df, actual_df, check_dtype=False, check_categorical=False
        )
    ...
    def test_load_sequencing_summary_data_col_types(self):
        ...
        actual_df = sse.SequencingSummaryExtractor(
            self.config
        )._load_sequencing_summary_data()
        assert pd.api.types.is_bool_dtype(actual_df["passes_filtering"])

        # test if all columns are filled with number values except passes_filtering & barcode_arrangement
        actual_df_types = actual_df.drop(
            ["passes_filtering", "barcode_arrangement"], axis=1, inplace=False
        ).dtypes
        assert all(pd.api.types.is_numeric_dtype(t) for t in actual_df_types)
    ...
    def test_load_sequencing_summary_data_barcode_data(self):
        ...
        for index, value in random_values.items():
    ...
    def test_init_whole_config(self):
        ...
        actual_df_channel = instance.dataframe_1d["channel"].head(11)

        actual_df_passes_filtering = instance.dataframe_1d["passes_filtering"].head(11)

        actual_df_sequence_length = instance.dataframe_dict[
...
class TestSequencingSummaryExtractorOnlySequencingSummary(unittest.TestCase):
    ...
    def test_extract_only_sequencing_summary(self):
        ...
        channel_max = instance.dataframe_1d["channel"].value_counts().max()
        # mean_length = pd.DataFrame.mean(instance.sequence_length_df)
        mean_length = instance.dataframe_dict["all.reads.sequence.length"].mean()
        # read_pass_length_min = pd.DataFrame.min(instance.sequence_length_df[instance.dataframe_1d['passes_filtering'] == True])
        read_pass_length_min = instance.dataframe_dict["all.reads.sequence.length"][
        ...
        self.assertEqual(
            read_count,
            actual_dict["basecaller.sequencing.summary.1d.extractor.read.count"],
        )
        self.assertEqual(
            read_pass_count,
            actual_dict["basecaller.sequencing.summary.1d.extractor.read.pass.count"],
        )
        self.assertEqual(
            read_fail_count,
            actual_dict["basecaller.sequencing.summary.1d.extractor.read.fail.count"],
        )
        self.assertEqual(
            read_pass_ratio,
            actual_dict["basecaller.sequencing.summary.1d.extractor.read.pass.ratio"],
        )
        self.assertEqual(
            read_fail_ratio,
            actual_dict["basecaller.sequencing.summary.1d.extractor.read.fail.ratio"],
        )
        self.assertEqual(
            read_pass_frequency,
            actual_dict[
        ...
        self.assertEqual(
            channel_max,
            actual_dict[
                "basecaller.sequencing.summary.1d.extractor.channel.occupancy.statistics.max"
            ],
        )
        self.assertEqual(
            mean_length,
            actual_dict[
                "basecaller.sequencing.summary.1d.extractor.all.read.length.mean"
            ],
        )
        # self.assertEqual(read_pass_length_min, actual_dict['basecaller.sequencing.summary.1d.extractor.read.pass.length.min'])
        self.assertEqual(
            read_pass_length_min,
            actual_dict[
                "basecaller.sequencing.summary.1d.extractor.pass.reads.sequence.length.min"
            ],
        )
        # testing.assert_series_equal(read_fail_qscore, actual_dict['basecaller.sequencing.summary.1d.extractor.read.fail.qscore'])
        testing.assert_series_equal(
            read_fail_qscore, instance.dataframe_dict["fail.reads.mean.qscore"]
        )
        self.assertEqual(
            read_fail_qscore_50,
            actual_dict[
                "basecaller.sequencing.summary.1d.extractor.fail.reads.mean.qscore.50%"
            ],
        )
...
class TestSequencingSummaryExtractorDirectory(unittest.TestCase):
    ...
    def test_check_conf_with_directory(self):
        ...
        with self.assertRaises(FileNotFoundError) as context:
            ...
            sse.SequencingSummaryExtractor(self.config)._is_sequencing_summary_file(
                self.config["sequencing_summary_source"]
            )
            self.assertTrue(
                (
                    False,
                    "No such file or directory "
                    + self.config.get("sequencing_summary_source"),
                )
