...
class uBAM_Extractor:
    ...
    def graph_generation(self, result_dict):
        ...
        add_image_to_result(
            ...
            images,
            time.time(),
            pgg.yield_plot(self.dataframe, self.images_directory),
        )
        add_image_to_result(
            self.quiet,
            images,
        ...
        add_image_to_result(
            ...
            images,
            time.time(),
            pgg.plot_performance(self.dataframe, self.images_directory),
        )
        add_image_to_result(
            self.quiet,
            images,
    ...
    def extract(self, result_dict):
        ...
        set_result_dict_telemetry_value(
            result_dict, "software.name", self.header["basecaller"]
        )
        set_result_dict_telemetry_value(
            result_dict, "software.version", self.header["basecaller_version"]
        )
        set_result_dict_telemetry_value(
    ...
    def _load_uBAM_file(self):
        ...
        uBAM_df["passes_filtering"] = uBAM_df["passes_filtering"].astype(
        ...
        if self.is_barcode:
            uBAM_df["barcode_arrangement"] = uBAM_df["barcode_arrangement"].astype(
                "category"
            )
        return uBAM_df
    ...
    def _process_record(self, rec, record_count):
        ...
        data = [
            ...
            float(record_count)
            if iso_start_time is None
            else timeISO_to_float(
                iso_start_time, "%Y-%m-%dT%H:%M:%S.%f%z"
            ),  # start time
