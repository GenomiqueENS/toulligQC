...
class SequencingSummaryExtractor:
    ...
    def init(self):
        ...
        # Rename barcode column to 'barcode_arrangement' if needed
        if self.is_barcode and self.barcode_colname == "barcode":
            self.dataframe_1d.rename(
                columns={"barcode": "barcode_arrangement"}, inplace=True
            )
        elif self.is_barcode and self.barcode_colname == "alias":
            self.dataframe_1d.rename(
                columns={"alias": "barcode_arrangement"}, inplace=True
            )
        ...
        if (
            "passes_filtering" not in self.dataframe_1d.columns
            or not self.default_threshold
        ):
            self.dataframe_1d["passes_filtering"] = np.where(
                self.dataframe_1d["mean_qscore"] > self.threshold_Qscore, True, False
            )
    ...
    def extract(self, result_dict):
        ...
        log_task(
            self.quiet,
            "Extract info from sequencing summary file",
            start_time,
            time.time(),
        )
    ...
    def graph_generation(self, result_dict):
        ...
        add_image_to_result(
            ...
            images,
            time.time(),
            pgg.sequence_length_over_time(self.dataframe_dict, self.images_directory),
        )
        add_image_to_result(
            self.quiet,
            images,
        ...
        add_image_to_result(
            ...
            images,
            time.time(),
            pgg.speed_over_time(self.dataframe_dict, self.images_directory),
        )
        ...
        try:
            ...
            if len(files) == 1 and self._is_sequencing_summary_file(files[0]):
                return pd_read_sequencing_summary(
                    files[0],
                    cols=sequencing_summary_columns,
            ...
            elif len(files) == 1 and self._is_sequencing_summary_with_barcodes(
                files[0]
            ):
                if self.is_barcode:
                    sequencing_summary_columns.append(self.barcode_colname)
                sequencing_summary_datatypes.update({self.barcode_colname: "category"})
            ...
            for f in files:
                ...
                if self._is_barcode_file(f):
                    dataframe = pd_read_sequencing_summary(
                        f,
                        cols=barcoding_summary_columns,
                        data_type=barcoding_summary_datatypes,
                    )
            ...
            if barcode_dataframe is None:
                ...
                missing_barcodes_count = (
                    dataframes_merged[self.barcode_colname].isna().sum()
                )
                ...
                dataframes_merged[self.barcode_colname] = dataframes_merged[
                    self.barcode_colname
                ].fillna("unclassified")
    ...
    def _is_barcode_file(filename):
        ...
        return header.startswith("read_id") and any(
            col in header for col in ["barcode_arrangement", "barcode"]
        )
    ...
    def _get_barcode_colname(self, filename):
        ...
        header_columns = set(header.strip().split("\t"))
        if self.use_alias_for_barcodes and "alias" in header_columns:
            self.barcode_colname = "alias"
        elif "barcode_arrangement" in header_columns:
            self.barcode_colname = "barcode_arrangement"
        elif "barcode" in header_columns:
